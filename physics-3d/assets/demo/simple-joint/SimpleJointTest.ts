...
export class SimpleJointTest extends Component {
    ...
    @property({ visible: function (this: SimpleJointTest) { return this.USE_TRANSLATE } })
    readonly deltaMotion = new Vec3();

    @property({ visible: function (this: SimpleJointTest) { return this.USE_TRANSLATE } })
    USE_LOOP = false;

    @property({ visible: function (this: SimpleJointTest) { return this.USE_TRANSLATE && this.USE_LOOP } })
    readonly displacement = new Vec3();

    @property
    USE_ROTATION = false;

    @property({ visible: function (this: SimpleJointTest) { return this.USE_ROTATION } })
    ...
    @property({ visible: function (this: SimpleJointTest) { return this.USE_SCALE } })
    readonly deltaScale = new Vec3();

    @property({ visible: function (this: SimpleJointTest) { return this.USE_SCALE } })
    USE_LOOP_SCALE = false;

    @property({ visible: function (this: SimpleJointTest) { return this.USE_SCALE && this.USE_LOOP_SCALE } })
    ...
    private readonly _initEluer = new Vec3();
    private _yoyoFlag = 1;

    start () {
        Vec3.copy(this._initEluer, this.node.eulerAngles);
    ...
    update (deltaTime: number) {
        ...
        if (this.USE_ROTATION) {
            ...
            if (this.USE_YOYO) {
                const n = this.node;
                let needUp = false;
                if (n.eulerAngles.y < -60 && this._yoyoFlag != 0) {
                    this._yoyoFlag = 0;
                    needUp = true;
                } else if (n.eulerAngles.y > 60 && this._yoyoFlag != 1) {
                    this._yoyoFlag = 1;
                    needUp = true;
                }
                if (needUp) {
                    this.deltaEuler.negative()
                    Quat.fromEuler(this._rot, this.deltaEuler.x, this.deltaEuler.y, this.deltaEuler.z);
                }
            }
        }
    ...
    onBtnRotate () {
        this.USE_ROTATION = !this.USE_ROTATION;
    }

    onBtnTranslate () {
        this.USE_TRANSLATE = !this.USE_TRANSLATE;
    }

    onBtnYoyo () {
        this.USE_YOYO = !this.USE_YOYO;
        this.node.eulerAngles = this._initEluer;
    }
